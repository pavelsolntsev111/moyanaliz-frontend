...
const faqItems = [
  {
    q: "Как работает расшифровка анализов онлайн?",
    a: "Вы загружаете PDF или фото бланка. Сервис распознаёт показатели, сравнивает каждый с референсными значениями из вашего же бланка и объясняет понятным языком: что это за показатель, в какую сторону он отклонился и почему такое отклонение бывает. Два показателя открываются бесплатно сразу после загрузки, остальные — после оплаты. Диагноз сервис не ставит и лечение не назначает: трактовать отклонения должен врач, который видит вас, а не только цифры.",
  },
  {
    q: "Сколько стоит расшифровка анализов и что входит в цену?",
    a: "Полная расшифровка одного бланка — от 199 ₽, разовый платёж, без подписки и автосписаний. Предварительный результат по двум показателям бесплатный: вы платите, только если решили открыть отчёт целиком. В отчёт входят разбор всех распознанных показателей, отдельный блок по отклонениям от референсов, общие рекомендации по питанию и образу жизни, список вопросов врачу и PDF на почту. Дороже стоят вариант с чат-консультантом (от 249 ₽) и наборы отчётов (от 299 ₽) — в наборе один разбор обходится дешевле.",
  },
  {
    q: "Какие анализы можно расшифровать?",
    a: "Общий и биохимический анализ крови, липидный профиль, гормоны щитовидной железы и половые гормоны, витамины и микроэлементы, ферритин и обмен железа, анализ мочи, копрограмму, коагулограмму, маркеры воспаления, а также серологию: гепатиты, ИППП, ВИЧ. Подойдёт любой лабораторный бланк, где есть название показателя, результат и референсы. По инфекционным тестам сервис только поясняет, что означает показатель: скрининг даёт ложноположительные результаты, а подтверждает и сообщает результат врач. Описательные заключения — гистологию, УЗИ, генетические отчёты — мы не разбираем.",
  },
  {
    q: "Анализы какой лаборатории подойдут — Инвитро, Гемотест, КДЛ, Хеликс?",
    a: "Подойдёт бланк любой лаборатории: Инвитро, Гемотест, КДЛ, Хеликс, Ситилаб, частные клиники, а также распечатка из районной поликлиники или стационара. Годится PDF или скриншот результатов из ЕМИАС и с Госуслуг. Оформление бланка роли не играет — важно, чтобы читались название показателя, результат и колонка референсных значений. Именно с этой колонкой мы сравниваем ваши цифры: методы и нормы у разных лабораторий отличаются, поэтому единой «правильной» нормы не существует.",
  },
  {
    q: "Можно ли расшифровать анализ по фото с телефона?",
    a: "Да, это самый частый сценарий. Принимаем PDF, JPEG, PNG, WebP и HEIC (формат айфона) размером до 20 МБ, снять бланк можно камерой прямо на странице. Снимайте при хорошем свете, целиком, без бликов и сильного наклона: если названия, результаты и референсы на снимке не читаются, часть строк не распознается. Многостраничный PDF из лаборатории загружайте как есть — он разбирается целиком, делить его на страницы не нужно.",
  },
  {
    q: "Нужна ли регистрация, чтобы расшифровать анализ?",
    a: "Нет. Ни аккаунта, ни пароля, ни подтверждения телефона: загрузили файл — сразу видите предварительный результат. Email спрашиваем уже после оплаты и только затем, чтобы прислать PDF-отчёт; он останется у вас копией, если закроете вкладку. Готовый отчёт открывается по персональной ссылке — она же и есть доступ к нему, поэтому не пересылайте её посторонним. Весь путь от бланка до расшифровки — загрузка файла и оплата.",
  },
  {
    q: "Как быстро я получу результат расшифровки?",
    a: "Предварительный результат появляется на экране обычно за 10–30 секунд после загрузки — сразу видны первые показатели. Полный отчёт чаще всего готов за 1–3 минуты после оплаты; на объёмных бланках с десятками показателей бывает дольше. Отчёт открывается прямо в браузере, а PDF дополнительно уходит на указанную почту. Держать страницу открытой и обновлять её не нужно.",
  },
  {
    q: "Учитываются ли пол и возраст при расшифровке?",
    a: "Да. Пол и возраст сервис берёт из шапки бланка, вводить их отдельно не нужно. На трактовку это влияет напрямую: у мужчин и женщин, у молодых и пожилых ориентиры для гемоглобина, ферритина, креатинина и гормонов различаются. Если этих данных в бланке нет, разбор идёт по общим ориентирам, и в отчёте это указано. Главной опорой в любом случае остаётся колонка референсов вашей лаборатории — она уже учитывает пол и возраст.",
  },
  {
    q: "Что если в анализе много показателей или бланков несколько?",
    a: "Количество показателей не ограничено: в отчёт попадут все, что удалось распознать на бланке. Если лаборатория выдала один многостраничный PDF, загружайте его целиком — он читается как единый документ. А разные исследования на отдельных бланках (кровь, моча, гормоны) — это отдельные загрузки и отдельные отчёты. Когда бланков сразу несколько, выгоднее взять набор отчётов: один разбор обойдётся дешевле.",
  },
  {
    q: "Можно ли расшифровать анализы ребёнка?",
    a: "Да, детский бланк загружается так же. Оговорка здесь важная: нормы у детей сильно зависят от возраста и меняются буквально по месяцам, поэтому мы опираемся на референсы, напечатанные в бланке, — лаборатории обычно приводят их уже под возраст ребёнка. Расшифровка поможет понять, что это за показатели и что вышло за границы. Что с этим делать, решает педиатр: отклонения у детей — всегда повод показаться врачу, а не менять питание или давать препараты самостоятельно.",
  },
  {
    q: "Можно ли расшифровать анализы при беременности?",
    a: "Да, но читать их нужно с поправкой на срок. При беременности часть показателей меняется физиологически: обычно снижается гемоглобин из-за роста объёма плазмы, повышаются СОЭ, фибриноген и D-димер, иначе оцениваются ТТГ и показатели обмена железа. Поэтому цифра вне привычных референсов далеко не всегда означает проблему — и наоборот, значение внутри границ не всегда спокойный знак. Мы поясним, что означает каждый показатель, а трактовать результат с учётом срока и течения беременности должен ваш акушер-гинеколог.",
  },
  {
    q: "Что именно будет в PDF-отчёте?",
    a: "Отчёт на несколько страниц. Сначала сводка: сколько показателей распознано и сколько из них вне референсов. Дальше разбор каждого показателя понятным языком и отдельный блок по отклонениям: что означает показатель, в какую сторону ушёл, на что обратить внимание. Затем общие рекомендации по питанию и образу жизни, перечень уточняющих исследований, которые обычно обсуждают с врачом, и список вопросов на приём. Препараты, дозы и схемы лечения в отчёте не назначаются — это делает только врач. PDF приходит на почту, его удобно распечатать и взять с собой.",
  },
  {
    q: "Можно ли расшифровать старый анализ и посмотреть динамику?",
    a: "Давность бланка роли не играет — разберём и прошлогодний. Чтобы увидеть динамику, загрузите бланки разных дат: по каждому будет свой отчёт, и показатели можно сравнить между собой (для нескольких бланков выгоднее набор отчётов). Если лаборатория сама свела несколько дат в один документ, разбор это учтёт. Оговорка одна: корректно сравнивать результаты одной лаборатории — у разных лабораторий отличаются методы и референсы, и разница методик легко выглядит как рост или падение показателя.",
  },
  {
    q: "Заменяет ли расшифровка консультацию врача?",
    a: "Нет. Мы не медицинская организация, не оказываем медицинских услуг, не ставим диагноз и не назначаем лечение. Сервис помогает разобраться, что означают цифры в бланке и с чем идти к врачу: на приём вы приходите не с непонятной таблицей, а с пониманием, какие показатели вышли за референсы и какие вопросы стоит задать. Решение о диагнозе и лечении принимает врач очно, с учётом жалоб, осмотра и истории болезни.",
  },
  {
    q: "Что делать, если отклонений много или результат непонятен?",
    a: "Начните с отчёта: он покажет, что именно вышло за референсы и насколько. Небольшие отклонения бывают из-за еды накануне, стресса, времени сдачи или особенностей методики лаборатории, но это не значит, что их можно игнорировать: насколько отклонение значимо, оценивает врач. Дальше — записаться на приём и взять распечатку с собой; расставить приоритеты может только врач. Если остались вопросы по самому отчёту, напишите в поддержку или воспользуйтесь чат-консультантом (платная опция). При резком ухудшении самочувствия — боль в груди, одышка, высокая температура, кровотечение — не ждите разбора и обращайтесь за медицинской помощью.",
  },
  {
    q: "Безопасно ли загружать анализы — что будет с моими данными?",
    a: "Файл передаётся по шифрованному каналу и хранится в защищённом облачном хранилище. Распознавание и разбор выполняются автоматически, в том числе силами внешних сервисов, которых мы привлекаем как обработчиков по договору; для рекламы данные никому не передаются, не продаются и в открытый доступ не выкладываются. Регистрация не нужна, ФИО отдельно мы не спрашиваем — а если оно напечатано на бланке, закрасьте его перед съёмкой, на расшифровку это не повлияет. Email нужен только для отправки PDF. Данные о здоровье относятся к специальной категории персональных данных, поэтому обрабатываем их с вашего согласия и в соответствии с 152-ФЗ; подробности — в политике конфиденциальности.",
  },
]
...
export function UploadStep({ onFileSelected }: UploadStepProps) {
  ...
  return (
    <div className="flex flex-col">
        ...
        <div className="mx-auto grid max-w-6xl gap-10 px-4 py-8 md:grid-cols-2 md:items-center md:py-10">
          ...
          <div className="relative z-10 min-w-0 animate-fade-up">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              Первые показатели — <span className="font-semibold text-foreground">бесплатно</span>. Полный отчёт — <span className="font-semibold text-foreground">разовая оплата, без подписки</span>
            </p>
      ...
      {/* ─── FAQ ─── */}
      {/* FAQPage-разметка: вопросы на главной были текстом, но без schema — поисковик их не видел */}
      <script
      ...
      <section className="border-t border-border py-14">
        <div className="mx-auto max-w-3xl px-4">
          <div className="flex flex-col items-center gap-5 rounded-2xl border border-primary/15 bg-primary/[0.04] px-6 py-7 text-center sm:flex-row sm:justify-between sm:text-left">
            <div className="flex flex-col items-center gap-4 sm:flex-row">
              ...
              <div>
                ...
                <p className="mt-0.5 text-sm text-muted-foreground">
                  Служба поддержки на связи и поможет с любым вопросом
                </p>
              </div>
            </div>
            <a
